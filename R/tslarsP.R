...
#' fitting models along the sequence, parallel computing is implemented on the 
#' \R level using package \pkg{parallel}.  Otherwise parallel computing for 
...
tslarsP.default <- function(x, y, h = 1, p = 2, sMax = NA, fit = TRUE, 
    ...
    out <- tslarsPFit(x, y, h=h, p=p, sMax=sMax, robust=FALSE, centerFun=mean, 
        scaleFun=sd, fit=fit, crit=crit, ncores=ncores, cl=cl, model=model)
