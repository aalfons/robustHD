...
tslarsP.default <- function(x, y, h = 1, p = 2, sMax = NA, fit = TRUE, 
    ...
    out <- tslarsPFit(x, y, h=h, p=p, sMax=sMax, robust=FALSE, centerFun=mean, 
        scaleFun=sd, fit=fit, crit=crit, ncores=ncores, model=model)
...
#' the index 0.
#' 
#' Parallel computing for obtaining initial data cleaning weights is 
#' implemented on the \R level via package \pkg{parallel}.  Since common robust 
#' regression function such as \code{\link[robustbase]{lmrob}} (the default) 
#' involve some random component, it is recommended to set the random number 
...
rtslarsP.default <- function(x, y, h = 1, p = 2, sMax = NA, 
        ...
        crit = "BIC", ncores = 1, cl = NULL, model = TRUE, ...) {
    ## call fit function with classical functions for center, scale, 
    ## correlation and regression
    call <- match.call()  # get function call
    call[[1]] <- as.name("rtslarsP")
    out <- tslarsPFit(x, y, h=h, p=p, sMax=sMax, robust=TRUE, 
        ...
        fit=fit, crit=crit, ncores=ncores, cl=cl, model=model)
    if(inherits(out, "grplars")) out$call <- call  # add call to return object
    out
}
