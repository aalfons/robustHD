...
#' this is set to \code{NA}, all available processor cores are used.  For each 
#' lag length, parallel computing for fitting models along the sequence is 
#' implemented on the \R level using package \pkg{parallel}.  Otherwise 
#' parallel computing for some of of the more computer-intensive computations 
#' in the sequencing step is implemented on the C++ level via OpenMP 
#' (\url{http://openmp.org/}).
