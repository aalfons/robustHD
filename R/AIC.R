...
AIC.penModel <- function(object, ..., k = 2) {
  # initializations
  res <- residuals(object, s=NULL)
  ...
  n * (log(2 * pi) + 1 + log(object$scale^2)) + object$df * k
}
...
# method for penalized models
bicSelect.penModel <- function(object, ...) {
  # compute BIC values for different tuning parameters
  values <- BIC(object, ...)
  # select the best model and return BIC object
  bic <- list(values=values, best=which.min(values))
  class(bic) <- "bicSelect"
  bic
}
