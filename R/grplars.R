...
#' default is to use the root mean squared prediction error 
...
#' \code{\link{.Random.seed}}), which is useful for prediction error 
#' estimation.  On parallel \R worker processes, random number streams are used 
#' and the seed is set via \code{\link{clusterSetRNGStream}}.
...
grplars.default <- function(x, y, sMax = NA, assign, fit = TRUE, 
    ...
    out <- grplarsFit(x, y, sMax=sMax, assign=assign, robust=FALSE, 
        centerFun=mean, scaleFun=sd, fit=fit, crit=crit, splits=splits, 
        cost=cost, costArgs=costArgs, selectBest=selectBest, seFactor=seFactor, 
