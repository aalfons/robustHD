...
#' @rdname predict.seqModel
#' @method predict penModel
#' @export

predict.penModel <- function(object, newdata, s = NA, ...) {
  ## initializations
  coef <- coef(object, s=s)  # extract coefficients
  d <- dim(coef)
  terms <- delete.response(object$terms)  # extract terms for model matrix
  if(missing(newdata) || is.null(newdata)) {
    ...
    # otherwise use the terms object to extract model matrix
    if(is.null(terms)) {
      # model is assumued to have an intercept: add a column of ones to the
      # new data matrix (unless it already contains intercept column)
      newdata <- addIntercept(as.matrix(newdata), check=TRUE)
      # check dimensions of new data
      p <- if(is.null(d)) length(coef) else d[1]
      if(ncol(newdata) != p) {
        stop(sprintf("new data must have %d columns", p))
